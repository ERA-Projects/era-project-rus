...
> - позволяет добавить кнопки в общий список стилизованных кнопок в разные диалоги главного меню, котрые указываются через битовый набор id:
> ```cpp
> enum eMenuList : int
> {
...
- ### плагин "ERA_MultilingualSupport.era" обновлён до версии 2.0:
	- Теперь название папок для языков основывваются на формате "iso-639-1";
	- для добавления собтвенного языка достаточно просто прописать название локали в json ключе:
	```
...
#### WoG Scripts:
- исравлена функция проверки допустимости здания для постройки в городе;
