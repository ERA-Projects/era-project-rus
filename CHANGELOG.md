...
#### Mixed Neutrals:
- добавлены настройки количеств отрядов и пресеты; 

#### Advanced Difficulties Mod:
- Обновлён до версии 1.043:
    - улучшены настройки бонусов нейтральных существ;
    - исправления бонусов нейтральных существ;


### Версия 2.282

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 2.3.1:
    - сильно ограничено количество тюрем на случайнах картах;
    - исправлен вылет при перегенрации случайных карт;
    - восстановлена работоспособность кнопки "Полная рандомизация настроек генерации объектов";

#### ERA ERM Framework:
- добавлена новая функция для моментального удаления препятствия на поле боя:
```
...
#### WoG Scripts:
- объект "Зал Смерти": теперь препятствия на поле боя исчезают мгновенно;


### Версия 2.281

#### HD Mod:
- Обновлён до версии 5.8 R14;

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 2.3.0:
    - регистрация расширителей объектов переведена на версионируемый API `RegisterObjectExtenderEx` с callback-функцией и явным описанием размера, шага и формата записей;
    - добавлена безопасная обработка расширителей, собранных с различными версиями структуры `UniqueObjectInfo`, а также записей, добавленных после первоначальной регистрации;
    - добавлена проверка данных регистрации, диапазонов памяти и повторной регистрации расширителей;
    - объект "Телега" теперь отображается в категории общих объектов карты, а не в категории сокровищ;
- обновлены плагины "Objects_CommonHotaObjectsPack.era", "Objects_CreatureBanksExtender.era" и "Objects_WoGObjectsExtender.era" для нового API расширителей объектов;

#### Other:
- обновлён SDK API расширителей объектов в файле `Tools/Era/SDK/ObjectExtenderAPI.hpp`;


### Версия 2.280

#### WoG:
- обновлён плагин "Assembly_MainPlugin.era" до версии 1.10.0:
    - добавлено автоматическое переименование записей модов в `Mods/list.txt` при переименовании папок модов с последующим перезапуском игры;
    - индикатор новой версии теперь мигает, если доступно обновление;
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 2.2.0:
    - диалог настроек генерации объектов разделён на категории: общие объекты карты, объекты обучения, доходы и ресурсы, Банки существ, жилища существ, тюрьмы и квесты, объекты WoG, сокровища и ящики Пандоры;
    - добавлена отдельная настройка вариантов ящиков Пандоры (существа, золото, опыт и заклинания), включая отображение доступных заклинаний;
    - добавлена отдельная настройка вариантов тюрем;
    - расширена поддержка страниц диалога для объектов, добавляемых через API расширителей объектов;
- обновлены плагины "Objects_CommonHotaObjectsPack.era", "Objects_CreatureBanksExtender.era" и "Objects_WoGObjectsExtender.era" для работы с обновлённым API категорий диалога генерации объектов;
- обновлён плагин "ERA_LocaleManager.era" до версии 3.1.2:
    - улучшено сохранение экспортируемых JSON-файлов с корректными переводами строк Windows;
- удалены устаревшие патчи `AfterWoG/no prisons on random maps`, так как поддержка вариантов тюрем теперь встроена в настройки генерации объектов;

#### Advanced Classes Mod:
- обновлён до версии 1.12:
    - исправлен возможный вылет при просмотре описания артефакта на экране героя;
    - исправлено неправильное название третьего улучшения существа для специализации героя;
    - исправлено неправильное отображение заклинания командира для командира-Грубияна;
    - мод теперь загружается после "WoG Graphic fix";
- исправлено отображение частей составных артефактов без дублирования описания;

#### Game Enhancement Mod:
- исправлено отображение описаний частей составных артефактов при использовании Advanced Classes Mod;

#### TrainerX:
- исправлено отображение подписи и значка школы магии в интерфейсе;
- Shift-клик по телескопу или портрету героя теперь скрывает всю карту только для выбранного игрока;

#### WoG Scripts:
- исправлена установка маны героя в 0 на втором раунде боя с новыми Банками существ;


### Версия 2.279

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 2.1.1:
    - исправлен редкий вылет при генерации объекта "Водяная Мельница";
    - исправлен редкий вылет при генерации заклинаний;
    - улучшена инициализация новых звуков объектов;
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.4.3:
    - исправлена возможность изменять состояние новых опций из внешних скриптов/плагинов:
    ```
    int (__stdcall *GetOptionValue)( const char *key);
    int (__stdcall *SetOptionValue)( const char *key, int value);
    ```
    - улучшена работа диалога настроек полосок со здоровьем отрядов в битвах;
    - исправлена логика загрузки игры из окна битвы;
- обновлён плагин "XXL.era":
    - исправлено сохранение настроек для некоторых шаблонов случайных карт;

#### Game Enhancement Mod:
- обновлён плагин "prima.dll":
    - исправлен редкий баг со стартом игры с максимальными Первичными Параметрами у героев;

#### ERA ERM Framework:
- добавлена инициализация некоторый значений в глобальные переменные для простоты взаимодействия со скриптами:
    ```
    i^maxSpellId^: хранит индекс последнего доступного заклинания для книги заклинаний;
    ```

#### Quick Saving Mod:
- исправлено присвоение имени файлам при сохранении игры модом;


### Версия 2.278

#### WoG:
- временно удалён плагин "Extended Object Limits.era";


### Версия 2.277

#### WoG:
- добавлен плагин "Extended Object Limits.era", позволяющий играть в карты, где увеличено стандартное количество объектов Карты Приключений;
- обновлён плагин "game bug fixes extended.dll":
    - добавлены испраления поведения ИИ из плагина SoD_SP;
    - исправлена сила некромантии для виртуальных ИИ-битв (SoD_SP);
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 2.1.0:
    - теперь в диалоге настроек генерации все объекты центрированы и имеют один размер;
- обновлён плагин "XXL.era": исправлено отображение настроек для шаблонов случайных карт;
- обновлён плагин "wog native dialogs.era":
    - изменён стиль меню WoG-опций;
    - исправлена невозможность стартовать игру на некоторых системах;
- обновлён плагин "Interface_MainMenuAPI.era" до версии 1.11:
    - добавлена поддержка стилей кнопок, зависящего от нейтрального фона Главного Меню;

#### Game Enhancement Mod:
- обновлён плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.10.0:
    - улучшен алгоритм показа подсказок для объектов на "ALT";
    - исправлено некорректное вычисление размера заднего фона подсказки;
    - исправлено отображение "галочек" статуса постройки в городах на Карте Приключений при отображении менее 7 городов;
    - удалён неисползуемый код;
    - исправлена редкая ошибка "Map Drawing Error", возникавшая при старте карты;


### Версия 2.276

#### WoG:
- обновлён плагин "XXL.era": исправлено отображение шаблонов случайных карт;


### Версия 2.275

## ВНИМАНИЕ. Папка мода "Era Scripts Rus" была переименована, поэтому данный мод необходимо заново включить в Менеджере Модов;

#### WoG:
- обновлён плагин "wog native dialogs.era":
    - теперь доступен перевод WoG-опций, добавляемых через "zsetup01.txt" и ".ers" файлы;
    - достаточно просто прописать название ключа в строке, чтобы сработала замена;
- весь текст WoG-опций перенесён в json формат;

#### WoG Scripts:
- весь текст WoG-опций перенесён в json формат;

#### ERA Scripts:
- объединены русская и английская версия мода в единую для удобства локализации;
- весь текст WoG-опций перенесён в json формат;
...
#### WoG:
- обновлён плагин "ERA_JsonOverrides.era" до версии 1.2.1:
    - исправлена замена текста для здания "хранилище ресурсов";

#### Game Enhancement Mod:
- обновлён плагин "prima.dll":
    - исправлено редкое чтение отрицательных значений первичных параметров героев;
    - исправлена возможность героям иметь значение Силы Магии и Знаний меньше 1;

#### ERA Scripts:
- полностью заменены все строки из ert-файлов на json значения;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - теперь все "Сопротивления", выдаваемые Опытом Существ считаются врождёнными, что позволяет их игнорировать артефактом "Сфера Уязвимости" - @SadnessPower;
- обновлён плагин "ERA_LocaleManager.era" до версии 3.1.1:
    - исправлен codepage для Вьетнамского языка;
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 2.0.1:
    - исправлен вылет при попытке сгенерировать карту с объектами, имеющими отрицательный тип или подтип;
- обновлён плагин "XXL.era":
    - исправлен случайный сброс настроек размера карты;
    - теперь для взаимодействия доступны только поддерживаемые шаблоны и их настройки;
...
#### WoG:
- обновлён плагин "wog native dialogs.era":
    - исправлен вылет при отображении кастомных типов картинок в диалоговых окнах выше 36, однако максимальное значение установлено до 100;


### Версия 2.271

#### WoG:
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.4.2:
    - исправлено восстановление настроек быстрой битвы;
    - исправлена возможность "перезапустить" карту и "загрузить игру" во время сражения против удалённого игрока-человека;
...
- ## обновление ядра era.dll до версии 3.9.31:
    - добавлена экспортируемая функция `int (__stdcall* RestartCurrentProcess)()`, позволяющая немедленно перезапустить игровой клиент. Возвращает "FALSE" в случае неудачи;
    - добавлена поддержка нового твика HD-Mod <Graphics.NewTrue32>, устанавливающего 32-бит палитру для спрайтов. Добавлено в версии HD Mod 5.7 R40 (пока что отключено по умолчанию); 
    - исправлен баг застывшей анимации атаки на карте приключений и флаг, что идёт битва;

#### WoG:
- обновлён плагин "wog native dialogs.era":
    - в меню WoG-опций добавлено окно поиска по названию, номеру и описанию опции:
        - изменён стиль диалога для повышения читабельности текста;
        - в левом нижнем углу добавлено окно поиска, позволяющее моментально находить подходящие опции по названию, номеру или тексту описания опции;
        - при клике на подходящего кандидата моментально переходит к опции, временно подсвечивая её;
    - значительно улучшены оригинальные сообщения игры:
        - теперь возможно отобразить любой def, а не только заданные типы (от 0 до 36), указав его имя прямо в коде ERM (для плагинов достаточно передать адрес строки или указатель на него):
        ```
        !!IF:Q2/^zmark1.def^/0/1/^text^;
        ```
        - теперь возможно анимировать любой def, указав номер кадра равный "-1" (работает и с оригинальными типами от 0 до 36):
        ```
        !!IF:Q2/^zmgc01.def^/-1/1/^text^;
        ```
        - теперь возможно отобразить любую картинку (pcx), указав её имя прямо в коде ERM (для плагинов достаточно передать адрес строки или указатель на него). Если картинка имеет тип Pcx16, то вторым аргументом передайте число "16":
        ```
        !!IF:Q2/^HPL000KN.PCX^/0/(PIC_TYPE_ART)/1/1/^text^;
        ```
        - теперь возможно отобразить любую ВНЕШНЮЮ картинку (bmp/jpg/png/pcx/pcx16), указав её ЛОКАЛЬНОЕ имя прямо в коде ERM (для плагинов достаточно передать адрес строки или указатель на него). Вторым аргументом необходимо указать имя внешнего файла (относительный путь тоже работает). Картинка будет иметь тип Pcx16:
        ```
        !!IF:Q2/^myExtraPic.pcx^/^_HD3_Data\Common\close16.bmp^/1/^text^;
        ```
        - добавлена экспортируемая функция, позволяющая задать текст для следующего показываемого диалога. Работает для любого внутриигрового диалога (0-8 картинок). После показа диалога, текст удаляется:
        `int (__stdcall* PrepareMessageBoxText)(char** textsBelow,char** rightClickHints, int size);`
    - для иконок морали и удачи добавлено числовое отображение бонуса/штрафа;
    - добавлен новый тип сообщений (11), отображающий контекст в панели состояния в течение заданного времени:
        - принимает те же аргументы, что и MessageBox (IF:Q);
        - поддерживаются текст и до 2-х картинок;
        - отображается в течение 5 секунд (5000 мс);
        - последней парой аргументов можно передать длительность и принудительное пересоздание диалога;
    - заменены диалоги взаимодействия с камнями могущества командиров на оригинальные, что убирает зависимость от "lib1.res/bmp" каталога;
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 2.0.0-alfa:
    - плагин отныне подключает модули для объединения в единую систему, что позволяет добавлять контент независимо от основных изменений;
    - следующие плагины теперь добавлены как подключаемые модули:
        - "Objects_WoGObjectsExtender.era" - отвечает за генерацию WoG-объектами;
        - "Objects_CreatureBanksExtender.era" - отвечает за добавление, расширение, управление и генерацию Банков Существ;
        - "Objects_CommonHotaObjectsPack.era" - отвечает за добавление и генерацию некоторых базовых HotA-объектов;
    - добавлено SDK по относительному пути "Tools\Era\SDK\ObjectExtenderAPI.hpp";
- обновлён плагин "ERA_JsonOverrides.era" до версии 1.2.0. Добавлена поддержка для замены наименований, связанных с городами:
    - замена названия и описания стандартных построек в городах:
    ```
    "era.towns.buildings.[building_id].name": string,
    "era.towns.buildings.[building_id].description": string
    ```
    - замена названия и описания уникальных построек в городах:
    ```
    "era.towns.[town_type].buildings.[building_id].name": string,
    "era.towns.[town_type].buildings.[building_id].description": string
    ```
    - замена названий типов городов и случайных имён самих городов на карте:
    ```
    "era.towns.[town_type].name": string,
    "era.towns.[town_type].names.[index]": string
    ```
- обновлён плагин "Assembly_MainPlugin.era" до версии 1.9.0:
    - добавлено исправление для совместимости с новой версией HD-mod;
    - восстановлена работоспособность на Windows XP;
- обновлён плагин "ERA_LocaleManager.era" до версии 3.1.0:
    - теперь при смене языка в главном меню предлагается автоматическим перезапустить игру;

#### HD Mod:
- Обновлён до версии 5.7 R40;

#### Game Enhancement Mod:
- обновлён плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.9.1:
    - исправлена логика подсчёта очков передвижения при построении пути;
- Переписан плагин "BattleSave.dll":
    - исправлены некорректные позиции и внешний вид героя при сохранении игры при нападении во время полёта и выгрузке с лодки;

#### TrainerX:
- на клавишу "F6" добавлена возможность просмотра проходимости объектов (как в редакторе карт);

#### ERA ERM Framework:
- добавлена функция `!?FU(AdvMap_SetInfoPanelText);`, моментально отображающая текст в панели состояния (но лучше использовать !!IF:M0/11/^text^, так как отличаются и шрифт, и центрирование):
```
!?FU(AdvMap_SetInfoPanelText);
!#VA(text:x) (timeToDisplay:x);
```
- добавлена функция `!?FU(AdvMap_SetInfoPanelDef);`, моментально отображающая любой def и его кадр (однако возможны редкие визуальные глитчи при подборе кучек ресурсов. Снова рекомендую использовать
```
!?FU(AdvMap_SetInfoPanelDef);
!#VA(text:x) (defName:x) (defFrameId:x) (timeToDisplay:x);
```
- добавлена функция `!?FU(PrepareMessageBoxText);`, устанавливающая текст для следующего показанного сообщения. Парами идут аргументы, отвечающие за текст под картинкой и на ПКМ;
```
...
#### ERA Scripts:
- достижения: подготовка к изменению уведомлений о достижениях;

#### Other:
- обновлены API по относительному каталогу "Tools/Era/SDK/";
- плагинам восстановлена работоспособность на Windows XP;


### Версия 2.269

#### WoG:
- добавлен Испанский язык в диалог выбора языков;
- обновлён плагин "game bug fixes extended.dll":
    - теперь герой сразу изучает заклинания в городе после использования заклинания "Городской Портал";
    - теперь герои автоматически обмениваются заклинаниями (вызов логики навыка "Грамотность"), когда город обучает кого-либо из них;
    - отключён учёт колдовства заклинаний для ИИ в теоретических битвах;
    - исправлен вылет при открытии диалога существ в битве;

#### Other:
- SD Mod Manager обновлен до версии 0.98.83:
    - исправлено отображение названий модов;
    - кнопка настроек категорий перемещена в нижний левый угол;
...
#### ERA:
    - ВНИМАНИЕ! примеры использования всех нововведений находятся в файле "/Help/Era manual/era iii changelog.txt" или просто кликните по версии ERA в главном меню игры;
- ## обновление ядра era.dll без изменения версии (3.9.30):
    - исправлено двойное срабатывание триггера "OnBattleRegeneratePhase";
    - исправлен баг, который приводил к бесконечной продолжительности заклинания "Бешенство";

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.6.0:
    - теперь в диалоге настроек отображается количество, тип и город юнитов для Жилищ Существ;
    - теперь в диалоге настроек отображается количество и тип юнитов для Банков Существ;
    - в диалоге настроек добавлено управление следующими объектами:
        - Жертвенный Алтарь;
        - Обломки;
        - Потерпевший Кораблекрушение;
        - Морской Сундук;
    - добавлены следующие ключи для Банков Существ:
        Раскрытие области карты на указанный радиус после разграбления:
        ```
        "RMG.objectGeneration.16.13.states.0.revealRadius": int
        ```
        Отображение текста при посещении Банка Существ, который уже разграблен:
        ```
        "RMG.objectGeneration.16.13.text.plundered": string
        ```
    - теперь опция "Полная Рандомизация" не затирает выбранные игроком настройки генерации карты;
    - исправлен алгоритм замещения свойств объектов разными модами (ранее работал объект из мода с самым низким приоритетом);
    - исправлен алгоритм генерации Жилищ Существ на 4 юнита (тип 20);
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.3.1:
    - исправлено потенциальное повреждение стека при вызове функций из сторонних плагинов;
- обновлён плагин "game bug fixes extended.dll":
    - теперь специализация по юнитам так же рекурсивно влияет на получение минимальной скорости юнитов при подсчёт запаса хода героя;
    - фикс расчёта удачи при открытии диалога информации о существе;
- добавлены текстовые файлы, полностью англифицирующие игру;

#### ERA ERM Framework:
- теперь функция ``!?FU(BattleStack_MakeActive);`` вызывает Фазу Регенерации;
...
#### WoG Scripts:
- Исправлена ошибка, из-за которой в некоторых случаях не удавалось отключить камни силы, если соответствующая опция была отключена;

#### Enhanced Henchmen:
- Исправлена ошибка, из-за которой кентавр не мог получить навык стрельбы, даже достигнув необходимого ранга;

#### ERA Scripts:
- Зомби-Трупоеды: Восстановлен для совместимости с внешними модами;

#### Other:
- SD Mod Manager обновлен до версии 0.98.81:
    - Добавлен перевод на китайский язык (спасибо автору: Archer30);
    - Улучшено сопоставление кодов языков (см. .json в lng, дайте знать, если нужно больше одного);
    - Теперь можно загружать переведенную информацию о документации (из файла mod.json<.LngCode>.md).;
    - Используется регистр символов для загрузки информации о файлах в модах (например, EraPlugins и eraplugins теперь находятся в одной папке для этой функции);


### Версия 2.266

#### WoG:
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.3.0:
    - прокрутка элементов теперь начинается с 1-й функциональной кнопки, а не 2-й;
    - позволяет регистрировать и удалять кнопки и через плагины:
        - Регистрирует callback-кнопку для функции без аргументов (__cdecl* callback) в секции "другие настройки". Первый аргументом передаётся уникальный ключ (строка) вашей кнопки, вторым -- название кнопки в диалоге (поддерживается json-ключ), третьим -- описание на ПКМ (поддерживается json-ключ), четвёртым -- id вызываемой ERM ф-ции. Может быть как число, так и ваша именованная функция. Этой же функцией можно изменить вашу кнопку в любой момент:
        ```cpp
        int (__stdcall *RegisterPluginCallbackButton)( const char *tag, const char *name, const char *description, void (__cdecl*callback)());
        ```
        - удаляет вашу (или иную) кнопку из диалога в процессе игры, если она вам там, почему-то, больше не нужна;
        ```cpp
        int* (__stdcall *UnregisterPluginCallbackButton)(const char *tag);
        ```
> [!NOTE]
> - Кнопки, добавленные через плагины, автоматически не удаляются при выходе с карты;
...
#### WoG Scripts:
- "Отключение Скриптовых Сообщений" добавлено в новый диалог системных опций;
...
#### ERA:
    - ВНИМАНИЕ! примеры использования всех нововведений находятся в файле "/Help/Era manual/era iii changelog.txt" или просто кликните по версии ERA в главном меню игры;
- ## обновление ядра era.dll без изменения версии (3.9.30):
    - теперь для плагинов доступно чтение значение json файлов на момент инициализации, а не только после события "OnAfterWog";

#### WoG:
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.2.1:
    - теперь доступно закрытие диалога на ту же горячую клавишу, что и открытие: "O";
    - небольшие исправления;
...
#### WoG:
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.2.0:
    - теперь дополнительные ERM-кнопки для диалога отображаются в порядке их добавления;
    - исправлена критическая ошибка в логике создания диалога, которая могла вести к утечкам памяти при закрытии окна битвы;
    - теперь во время битвы нажатие на кнопку "Начать Заново" не закрывает диалог (если востребовано, сделаю то же и для Карты Приключений);
- обновлён плагин "ERA_JsonOverrides.era" до версии 1.1.0:
    - переписан алгоритм работы с ключами json;
    - добавлена поддержка инициализации данных заклинаний следующими ключами:
    ```
    ...
    ```
    - исправлена некорректная инициализация некоторых параметров артефактов;

#### Advanced Classes Mod:
- добавлен недостающий плагин "Combat_BallisticsIncreasesShootingRange.dll", который увеличивает дальность прямой стрелы от навыка "Баллистика";
- добавлен недостающий плагин "Combat_LeadershipDecreasesFactionsNum.dll", который виртуально уменьшает количество фракции в армии героя с навыком "Лидерство";

#### ERA Scripts:
- опция "Дополнительные Апгрейды": теперь не отключается, если сторонние моды добавляют существ;
- опция "Принадлежность Нейтралов": теперь не отключается, если сторонние моды добавляют существ;


### Версия 2.263

#### WoG:
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.1.0:
    - для настроек со множественным выбором изменена графика;

#### ERA Scripts:
- опция "Зомби-трупоеды": временно отключена совместимость с модами, добавляющими существ;


### Версия 2.262

#### WoG:
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.0.4:
    - исправлено мгновенное несрабатывание изменение настроек сетки боя;
    - исправлено некорректное сохранение выбранного режима быстрой битвы;
    - исправлено игнорирование настроек быстрой битвы;

    
### Версия 2.261

#### WoG:
 - обновлён плагин "EraEditor_JsonOverrides.dll" до версии 1.1:
    - добавлена поддержка локализации для стандартных объектов Карты Приключений;
    - добавлена поддержка локализации для Банков Существ;

#### Game Enhancement Mod:
#### ERA Scripts:
#### WoG Scripts:
- исправлена проверка наличия HD-Mod;


### Версия 2.260

#### WoG:
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.0.3:
    - теперь количество регистрируемых callback-кнопок для ERM-функций не ограничивается 3 (тремя). Добавлен слайдер для их прокрутки;
    - исправлена визуальная ошибка при отрисовке свитча на 10 элементов;
    - добавлены экспортируемые функции для управления состоянием новых настроек извне. Название ключей хранится в файлах "heroes3.ini" и "default heroes3.ini" в секции "[Settings.Extra]":
    ```
    int (__stdcall *GetOptionValue)( const char *key);
    int (__stdcall *SetOptionValue)( const char *key, int value);
    ```
...
#### WoG Scripts:
- исправлен вылет при просмотре Вог-опций;

#### Game Enhancement Mod:
- удалено фантомное сообщение при клике ПКМ на Карте Приключений;


### Версия 2.258

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.5.3:
    - добавлена возможность добавлять и изменять звуки окружения для декоративных объектов;
    - исправлено логика ИИ для посещения некоторых новых объектов;
...
#### WoG Scripts:
- опция "Настройки быстрой битвы" (233) удалена;
...
#### WoG:
- обновлён плагин "ERA_SystemOptionsExtension.era" до версии 1.0.2:
    - выровнено положение элементом меню по горизонтали;
    - обновлён перевод;
- обновлён плагин "XXL.era":
    - исправлен вылет при вводе чита на открытие/сокрытие всей карты;
- добавлены звуки посещения wog-объекту "Сфинкс" и некоторым wog-банкам существ;

#### Advanced Classes Mod:
- мод обновлён до версии 1.11:
    - Исправлено поведение случайности в бою и описании.
    - Немного снижено максимальное количество присоединяющихся с помощью навыка "Дипломатия";
    - Командир-лидер теперь отображает масштабирование эффекта ауры в описании;
    - Вторичный навык "Баллистика" теперь уменьшает штраф к дальности стрельбы для стрелков. -3 на уровне мастера;
    - Вторичный навык "Лидерство": удалён дополнительный урон после срабатывания морали;
    - Вторичный навык "Лидерство": позволяет добавлять в армию больше юнитов иных фракций. +3 на уровне грандмастера;
    - Навык искателя приключений "Грабитель": теперь в 50% случаев возвращает очки передвижения, а в остальных 50% случаев увеличивает количество подобранных ресурсов;
...
#### ERA:
    - ВНИМАНИЕ! примеры использования всех нововведений находятся в файле "/Help/Era manual/era iii changelog.txt" или просто кликните по версии ERA в главном меню игры;
- ## обновление ядра era.dll до версии 3.9.30:
    - Добавлен булевый параметр в heroes3.ini: "Text.OldClosingBracketOpt". Он включает старое, ошибочное поведение отрисовки текста, где символ "}" использовался для закрытия всех открытых тегов. Он будет удален (не будет поддерживаться) в Era 4+. Пример:
    ```
    "{Это {золотое {сообщение} и это белая часть текста".
    ```
    - Добавлен новый целочисленный параметр в heroes3.ini: "CodePage". Если он задан, он указывает пользовательскую кодовую страницу (кодировку), которая будет использоваться для декодирования JSON-файлов на языке UTF-8. По умолчанию используется системная кодовая страница. Пример: 1251 для Windows-1251 (кириллица).
    - Добавлены следующие экспортируемые функции в era.dll:
        - Возвращает строку текущего языка/локали. Буфер необходимо освободить, вызвав Era.MemFree:
        ```
        function GetLanguage: {O} pchar; stdcall;
        ```
        - Изменяет кодовую страницу (кодировку), используемую для декодирования строк UTF-8 в JSON-файлах языков:
        ```
        function SetCodePage (NewCodePage: cardinal): TInt32Bool; stdcall;
        ```
        - Возвращает текущую кодовую страницу (кодировку), используемую для декодирования строк UTF-8 в JSON-файлах языков:
        ```
        function GetCodePage: cardinal; stdcall;
        ```
    - исправлено некорректное чтение ini-файлов, у которых строка может быть без значения;
- обновление ядра eramap.dll (для редактора карт);
    - теперь "eramap.dll" поддерживает почти все экспортируемые функции, что и era.dll;
    - редактор карт теперь читает язык для json из файла "heroes3.ini";
    - добавлена SDK: "Tools/Era/SDK/eramap.h" и "Tools/Era/SDK/eramap.cpp";

#### WoG:
- ## добавлен совершенно новый плагин для настроек игры "ERA_SystemOptionsExtension.era":
    - добавляет унифицированное окно для всех настроек, заменяя оригинальные окна "системных опций" и "опций битвы" (размер диалога сохранён до пикселя, так что доступ к основным кнопкам не повлияет на мышечную память);
    - новые опции, которые имеет диалог настроек:
        - включение/отключение плавной прокрутки Карты Приключений;
        - включение/отключение очереди битвы существ (нельзя изменить в бою);
        - включение/отключение фоновых звуков объектов (зацикленные которые);
        - включение/отключение автосохранения игры;
        - включение/отключение альтернативной механики и звуки клика;
        - переключение типом быстрой битвы, используемого по умолчанию;
        - включение/отключение и управление полосками здоровья существ в бою;
        - расширенный диапазон скоростей анимации в битве;
    - а ещё диалог позволяет:
        - выйти в любое меню игры из битвы (да, даже загрузиться);
        - просмотреть WoG-опции в любой момент игры;
        - сменить язык игры (не для всех игровых данных это изменит текст без перезапуска);
        - управлять настройками других модов (если авторы их туда добавят, да-да);
    - предоставляет следующие экспортируемые функции:
        - Регистрирует callback-кнопку для ERM-функции в секции "другие настройки". Первый аргументом передаётся уникальный ключ (строка) вашей кнопки, вторым -- название кнопки в диалоге (поддерживается json-ключ), третьим -- описание на ПКМ (поддерживается json-ключ), четвёртым -- id вызываемой ERM ф-ции. Может быть как число, так и ваша именованная функция. Этой же функцией можно изменить вашу кнопку в любой момент:
        ```cpp
        ...
        ```
- ## добавлен новый плагин "XXL.era" (@JackSlatter):
    - переписанный плагин для поддержки карт большого разрешения;
    - огромное количество улучшений и исправлений для экрана выбора карт и генерации случайных карт;
    - добавлены XXL шаблоны для генрации случайных карт;
- ## добавлен совершенно новый плагин для РЕДАКТОРА КАРТ "EraEditor_JsonOverrides.dll":
    - заменяет строки некоторых txt-файлов значениями из json (на данный момент работает только с артефактами);
    - убирает требование к текстовому файлу со списком артефактов всегда быть конкретного размера ("artraits.txt" файл теперь можно удалить из модов);
    - позволяет полностью изменить значения для артефактов со следующими ключами:
    ```
    ...
    ```
- ## плагин "ERA_MultilingualSupport.era" разделён на 2 раздельных плагина:
    - "ERA_LocaleManager.era" - опубликован сразу с версией 3.0:
        - позволяет выбрать язык для чтения json-ключей;
        - теперь меняет и кодировку (ERA 3.9.30), что позволяет отображать корректные символы для выбранного языка, если не установлен режим "UTF-8" для игры;
        - теперь позволяет экспортировать текст из игры в подпапку "/Runtime/Exports/[locale_name]/"
        - предоставляет следующие экспортируемые функции:
            - Вызывает диалог выбора языка по указанным координатам с определённым стилем:
            ```cpp
            void (__stdcall *CallLocaleSelectionDlg)( int x, int y, int styleIndex);
            ```
            - Возвращает текущее отображаемое название для выбора языка (возврат идёт в текстовый буфер игры, так что память очищать не надо);
            ```cpp
            char* (__stdcall *GetDisplayedName)();
            ```
    - "ERA_JsonOverrides.era" - опубликован с версией 1.0:
        - заменяет строки некоторых txt-файлов значениями из json;
        - позволяет полностью изменить значения для артефактов со следующими ключами:
        ```
        "era.artifacts.%d.name":                    string,
        "era.artifacts.%d.cost":                    int,
        ...
        ```
- обновлён плагин "game bug fixes extended.dll":
    - из названия случайной карты удалены лишние символы, добавляемые при её генерации с включённым HD-Mod;
    - исправлено колдовство в автоматической битве, даже если настройка "колдовство в автобитве" отображалась отключённой;
    - исправлен показ выбора настроек игрока при просмотре карт;
    - исправлено влияние смены музыки в бою на генератор случайных чисел в бою;
    - исправлено влияние анимации "ожидания" на генератор случайных чисел в бою;
    - исправлено влияние просмотра диалога с Боевыми Машинами на генератор случайных чисел в бою;
    - добавлено исправлении поведения ИИ, заданного через ERM, на больших картах;
    - добавлено исправление поведения ИИ в битве против ИИ;
- обновлён плагин "wog native dialogs.era":
    - теперь в окне просмотра информации о существе проигрывается анимация даже при его вызове на ПКМ;
    - теперь в окне просмотра жилища существа проигрывается анимация;
    - теперь зажатие клавиш на клавиатуре не вызывает зацикливание нажатий кнопок интерфейса, к которым те привязаны;
    - удалён звук "щелчок мыши" при клике на навыках командира при повышении уровня;
- обновлён плагин "Interface_MainMenuAPI.era" до версии 1.10:
    - исправлена невозможность отключить базовые добавляемые кнопки через json;
- оптимизированы и переименованы некоторые файлы:
    - "era_main_menu_panel.pac" -> "era_InterfaceAssets.pac";
    - "rmg_settings_dlg.pac" -> "era_InterfaceAssets.pac";
    - обновлён архив "wog_lang.zip" до актуальной версии;
    - исправлен порядок ключей в json-файлах, содержащих названия существ;

#### Advanced Classes Mod:
- мод обновлён до версии 1.1:
    - Шанс убийства Огненного Мага теперь снова составляет 30% при атаке, а не зависит от количества убийств (в некоторых условиях он был слишком высоким);
    - Исправлено описание Огненного Мага;
    - Немного снижен шанс получения заклинания "Орлиный Глаз" на ранних уровнях (-10% шанс);
    - Исправлено отображение статистики Дипломата на экране героя при нажатии на навык, если навык был 9-го или 10-го уровня;
    - Исправлено отображение всплывающего окна выбора Некроманта на экране героя при нажатии на навык, если навык был 9-го или 10-го уровня;
    - Список бонусов (героя) в бою теперь отображается только при нажатии CTRL+ЛКМ или CTRL+ПКМ;
    - Переписаны Аура Командования, Аура Командира-Предателя, Аура Генерала Командира, улучшена их настраиваемость в файле erm и расширен диапазон по умолчанию (код от Yuritsuki, спасибо!);
    - Временно отключен эффект Мастерского Рассеивания в бою, так как он может испортить значения скорости для всех игроков;

#### Game Enhancement Mod:
- настройки мода теперь перенесены в новый диалог настроек игры;
- следующие механики удалены из мода, так как их логика перенесена в новый плагин "ERA_SystemOptionsExtension.era":
    - переключение типа битвы;
    - плавная прокрутка карты;
    - отображение полосок здоровья юнитов в бою;
    - удалены все кнопки настроек из интерфейса Карты Приключений;
- восстановлен размер и положение кнопки "Следующий герой";
- удалён фикс, изменяющий ИИ-ценность существ на старте карты;
- удалены сообщения об ошибках при вызове функций, связанных с элементами диалога, если id элемента был меньше нуля;

#### WoG Scripts:
- Улучшенные Военные Машины III: исправлено положение отображения текущего количества машин;
- Неделя монстров: улучшен алгоритм выбора типа монстров;
- Хижины командиров ведьм: исправлена картинка отображаемого навыка;
- прочие исправления;

#### ERA Scripts:
- опция "Ограничение количества героев": исправлена возможность для ИИ-игроков покупать героев выше лимита;
- опция "Достижения": исправлен цвет рамки всплывающих диалогов;
- опция "Зомби-трупоеды": добавлена совместимость с модами, добавляющими существ;
- механика "просмотр героя в таверне на ПКМ без схлопывания окна" отключена из-за неудобства;

#### WoG Graphics Fix Lite:
- актуализирована графика до версии оригинального мода 2.26.0;

#### XXL:
-  мод удалён, так как его логика перенесена в новый плагин "XXL.era" в моде "WoG";

#### BattleSpeed:
-  мод удалён, так как его логика перенесена в новый плагин "ERA_SystemOptionsExtension.era" в моде "WoG";

#### Other:
- SD Mod Manager обновлен до версии 0.98.79:
    - При открытии папки mods будет открываться папка mods, а не папка managed.
    - Добавлена опция (локальная для платформы) "Использовать устаревшее отключение модов". Использование этой опции удалит состояние "Отключено" для модов (т.е. в MM будет Enabled/Disabled/Archived, в старых версиях MM были только Enabled и Archived (а Archived называлась Disabled), но новые названия Enabled/Archived останутся без изменений);
    - Исправлена проблема с загрузкой пресетов, созданных до введения fold_case для идентификаторов модов.
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - исправлена функция получения опыта существ при корректировке ранга; 

#### TrainerX:
- исправлен вылет при попытке показать описание свитка с заклинанием;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - исправлена поломка опыта существ при вызове ERM-команд HE:C и EX:C;
    - исправлена ошибка, при которой цель навыка "Кавалерийский бонус" при расчёте ИИ-игроком некорректно определялась для последующих функций;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - Для ИИ-ИГРОКОВ исправлена потеря и поломка опыта существ при улучшении отрядов в городе;
    - Для ИИ-ИГРОКОВ исправлена потеря и поломка опыта существ при улучшении отрядов в Форте на Холме;
    - Теперь Командир не помещает баллисты для героя, защищающегося в стенах города (оригинальная механика игры);
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.5.3:
    - Для "Жилищ Существ" добавлена возможность устанавливать базовую ценность объекта (ранее была зависимость от ИИ-ценности живущих там существ. Вот так можно установить базовую ценность для Заоблачного Храма, не опираясь на ИИ-ценность живущих в нём Гигантов:
    ```
    "RMG.objectGeneration.17.44.value": 1234
    ```
    - Теперь диалог настройки объектов для генерации доступен и без HD-Mod (но требуется наличие кнопки кнопки в главном меню);
    - Теперь по умолчанию отключена генерации Палаток Ключника и Стражей Границы;

#### WoG Scripts:
- правило карты "Автоматическое улучшение существ 7-го уровня": исправлено сильно завышенное количество опыта у улучшенного отряда;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - юниту "Горынычи" добавлен флаг "Король 1", что повышает атаку против данного отряда, если на атакующий под действием заклинания "Палач";

#### WoG Scripts:
- опция "Рост популяции нейтральных городов": исправлена проверка на особый флаг при увеличении охраны и награды Банков Существ;

#### Other:
- SD Mod Manager обновлен до версии 0.98.77:
    - Нажатие клавиши F5 также перезагружает файлы описания модов;
    - возвращена поддержка Windows 7;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - Теперь стартовая настройка и покупка героев сразу корректирует Опыт их армий;
    - Для ИИ-ИГРОКОВ исправлена потеря и поломка опыта для в следующих случаях:
        - при посещении города;
        - при покупке существ в городе;
        - при обмене армией с городом;
        - при обмене армией с другим героем;
        - при обмене армией с гарнизоном;
        - при покупке существ вне города;
        - при покупке героя;
        - при получении существ в награду;
        - при присоединении существ на карте;
    - Фанатикам Войны возвращена способность двойного выстрела;
- обновлён плагин "wog native dialogs.era":
    - в диалоге просмотра Опыт Существ для кнопки "Просмотр улучшенного существа" отключена горячая клавиша "Пробел";

#### ERA ERM Framework:
- добавлена инициализация некоторый значений в глобальные переменные для простоты взаимодействия со скриптами:
    ```
    i^maxArtifacId^: хранит индекс последнего доступного артефакта;
    i^maxHeroId^:    хранит индекс последнего доступного героя;
    i^maxMonsterId^: хранит индекс последнего доступного монстра;
    ```

#### WoG Graphics Fix Lite:
- актуализирована графика до версии оригинального мода 2.25.1;

#### TrainerX:
- исправлена ошибка при открытии окна чужого героя, если у текущего игрока уже максимальное количество героев;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - исправлена способность "Присоединение отрядов нейтральной армии", если битва ведётся между войсками двух игроков. К тому же исправлена вызываемая рассинхронизации в подобном бою;
    - во всех базовых диалогах игры исправлено отображение юнитов если их количество превышает 99999;
- удалены добавленные в WoG панели ресурсов, используемые без HD Mod;

#### WoG Scripts:
- исправлено помещение объекта "Случайный монстр" при замене некоторыми объектами других;
- небольшая оптимизация скриптов;

#### Game Enhancement Mod:
- актуализировано описание мода;
...
#### ERA:
    - ВНИМАНИЕ! примеры использования всех нововведений находятся в файле "/Help/Era manual/era iii changelog.txt" или просто кликните по версии ERA в главном меню игры;
- ## обновление ядра era.dll до версии 3.9.29:
    - Расширенный синтаксис SN:F с возможностью вызова произвольных функций плагинов или DLL: `SN:F^plugin_name:function_name^/...`.
    Если имя плагина указано без точки, то расширения ".era" и ".dll" проверяются по очереди. В противном случае используется имя без изменений. Результат кэшируется.
    Пример:
    ```
    SN:F^era:ShowMessage^/^Hello, world!^;
    ```
    Даже сам файл 'era.dll' можно вызвать таким образом, хотя в этом случае рекомендуется вообще не указывать имя DLL-файла.    
    
    - Исправлена ошибка оригинальной игры: обработка таймера реального времени зависела от того, был ли компьютер активен менее 23 дней (переполнение в функции timeGetTime).

#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - исправлено зависание игры при проверки улучшенных версий существ у некоторых героев-специалистов по существам;
    - исправлено некорректное определение владельца способностей Опыта Существ, при попытке колдовать после атаки;
    
#### Game Enhancement Mod:
- обновлён плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.8.3:
    - теперь новая подсказка о статусе постройки зданий не отображается при ПКМ на вражеских городах (но оставил этот эффект с максимальным эффектом заклинания "Видение";
    - теперь новая подсказка о статусе постройки зданий работает и в "Обзоре Королевства";

#### ERA Scripts:
- опция "Исправление Поведения ИИ" удалена из-за неработоспособности;

#### Other:
- SD Mod Manager (@SyDr) обновлен до версии 0.98.75:
    - Исправлена проблема, из-за которой информация о совместимости не загружалась для заархивированных модов
    - Отправка мода в архив также отключала зависимые моды
    - Обновлен набор инструментов MSVC до версии 145
    - Английский язык будет загружаться, если выбранный язык не может быть загружен
    - При сортировке модов их имена будут сравниваться без учета регистра
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - временно отключена последнее изменение с поддержкой выстрела Магогов из-за несовместимостью со скриптами;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - Полностью переписана функция и логика работы  не наносящих урон заклинаний, которые реализованы WoG - существами и Опытом Существ: к после атаки, расчёт которых производился до самой атаки:
        - Теперь актуальность заклинания и возможность наложить заклинание просчитывается уже после совершения действия;
        - Теперь используется внутриигровая логика для проверки иммунитетов к накладываемым заклинаниям, что позволяет управлять их наложением через сторонние плагины и скрипты;
        - Теперь есть возможность наложить несколько заклинаний после атаки, выдаваемых способностью "99", а не только 1 (ранее работало последнее в списке);
        - Теперь способность опыта "Не наносящее урона заклинание после атаки" доступна для Пожары и Волшебниц;
    - Теперь атаки Магогов (атаки по площади огненным  шаром) вызывают срабатывание триггера "OnAfterShoot" (Решение для атаки облаком смерти Личей будет реализовано позднее);
    - Улучшено автоматическое применение заклинаний в бою:
        - теперь и автоматический, и быстрый бой учитывают внутриигровую опцию авто-битвы "применения заклинаний" для игрока-человека, что исправляет несоответствие между быстрой и автоматической битвой, если доступно колдовство;
- обновлён плагин "wog native dialogs.era":
    - небольшие исправления для совместимости с последними изменениями;

#### Game Enhancement Mod:
- изменён скрипт выбора типа битвы для совместимости с последними изменениями;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - теперь ИИ-игрок корректно использует способность "Кавалерийский бонус", приобретённый через опыт существ, навыки комаднира или сторонние плагины, использующие в логике ф-цию "int IsChampion()";

#### ERA Scripts:
- возвращена старая опция "Дополнительные улучшения" на 6-ю страницу:
    - теперь данная опция добавляет лишь недостающие улучшения из подобного скрипта из мода "WoG Scripts";
    - теперь автоматически улучшенным существам не будет присваиваться принадлежность какому-либо городу - используйте опцию 821 "Принадлежность нейтралов", которая расположена рядом;
    - теперь опция не работает, если максимальное количество существ изменено;
- Принадлежность нейтралов: теперь опция не работает, если максимальное количество существ изменено;
- Третий Класс: исправлена ошибка при нажатии на пустой навык в окне повышения уровня героя;
- немного изменён порядок опций:
    - опция "Принадлежность нейтралов" перенесена на 6-ю страницу;
    - опция "Боевой опыт" перенесена на 7-ю страницу;

#### Other:
- обновлён Sublime Text до версии 4200;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - теперь заклинание "Берсерк", накладываемое в результате ответного удара или выстрела не будет сбрасываться в конце действия, чтобы не терять эффект;
    - теперь специалисты по существам дают бонус не только улучшенной версии своих существ, но и любому другому улучшению, которое за ним последует (цикличная проверка);
    - небольшое исправление логики атаки стен для существ, не имеющих данного флага по умолчанию, но добавленного модами;
- новые обои теперь запакованы в pac-файле, позволяя тем замену другими модами;

#### Game Enhancement Mod:
- обновлён плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.8.2:
    - переписан код для отображения мифрила;
    - теперь мифрил виден во всех диалогах с отображением панели ресурсов;
    - изменён тип отображения ресурсов выше 1 млн;
    - исправлена подсказка при наведение на ресурсы;
    - исправлена подсказка показа длины пути при нажатии клавиши "ALT";
- небольшая оптимизация скриптов;
...
#### WoG:
## - добавлен официальный фон главного меню для HoMM 3 ERA:
    - художник - Rizzon;
...
#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.5.2:
    - исправлен редкий баг с некорректным индексом Банка существ;

#### WoG Graphics Fix Lite:
- актуализирована png-графика;


### Версия 2.241

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.5.1:
    - исправлен вылет при попытке получить награду за победу в Склепе/Покинутом Корабле/Кораблекрушении;
    - исправлено получение кастомных наград;
    - повышена стабильность плагина;


### Версия 2.240

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.5.0:
    - для Банков Существ улучшена загрузка данных из JSON файлов;
    - для Банков Существ оптимизировано поведение ИИ при расчёте ценности Банков Существ;
    - для Банков Существ добавлена поддержка сообщений о "победе";
    - для Банков Существ исправлена ошибка с наградой в виде существ, если данные отсутствовали;


### Версия 2.239

#### Game Enhancement Mod:
- из "первоапрельской шутки" удалено понижение радиуса разведки героев:
    ## НАЖМИТЕ "F12", ЧТОБЫ ПРИМЕНИТЬ ИЗМЕНЕНИЯ В УЖЕ НАЧАТОЙ ИГРЕ!
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    ## Путём долгих исследований и кропотливой работы в игру добавлена поддержка последовательной и корректной работы ГСЧ (Генератора Случайных Чисел) для сражений:
        - теперь результат "быстрой битвы" и "автоматической битвы" всегда идентичен, если в процессе битвы не участвуют скрипты, основанные на "времени" (VR:T);
        - теперь урон и срабатывание способностей всегда повторяются при соблюдении последовательности действий;
        - повышена производительность ГСЧ;
        - подготовка платформы к работе с одновременными анимациями всех существ;
    - теперь при полном блокировании атаки не пишется текст о полученном уроне в логе битвы;
- обновлён плагин "wog native dialogs.era":
    - теперь при получении войск благодаря навыку Командира Инферно (Суккуба) не будет отображаться избыточное сообщение об очаровании отрядов. Текст перенесён непосредственно в диалог присоединения армий;
    - изменена механика "быстрой битвы", вызываемой на "Q". Теперь мана героев не отнимается на время сражения, а принудительно включается оригинальная опция на запрет колдовства;
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.4.2:
    - в диалоге настроек исправлен предпросмотр крупных объектов;
- исправлена графика для объекта "Колизей Магов";

#### WoG Scripts:
- Улучшенные Военные Машины III: исправлено отображение текущего количества машин в окне встречи героев при их передаче передаче;
- исправлено зацикливание событий, вызванное некорректным управлением ГСЧ;
- мелкие исправления;
...
#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.4.1:
    - частичная подготовка к разделению плагина на несколько;
    - повышена производительность плагина;
    - исправлено застревание ИИ-игроков при посещении Банков Существ;
    - восстановлена работоспособность кастомных Университетов и некоторых других новых объектов;
    - улучшен алгоритм генерации Жилищ Существ с 4 существами (тип 20);
    - теперь в Диалоге Настроек РМГ объекты отображаются не из файла "zaobjts.txt", а "zobjcts.txt".
...
#### Game Enhancement Mod:
- обновлён плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.7.5:
    - обновлена подсказка, отображающая количество таверн, дающих информацию в окне Гильдии Воров:
        - добавлена поддержка вызова через HD Mod на клавишу "G";
        - добавлена локализация;
        - исправлено положение и размер подсказки;
- небольшие исправления переводов;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - теперь при полном блокировании урона отрядами не будет отображаться сообщение о нанесённом уроне;

#### Advanced Classes Mod:
- мод обновлён до версии 1.1:
    - Добавлено 2 новых уровня для каждого улучшения и зачарования боевой машины, всего 3. Также введены ультимативные улучшения/зачарования боевой машины. Их сложно получить, и они ограничены.
    - Введен новый параметр "Эффективность боевой машины", который можно получить, улучшая свою боевую машину, некоторые навыки, героев или артефакты.
    - Добавлены четыре новых счетчика характеристик: убитых рейдовых боссов, убитых боевых командиров, захваченных банков существ, зачищенных арен. (В следующем патче я добавлю еще больше статистики)
    - Переработан список бонусов и добавлено больше описаний при щелчке правой кнопкой мыши по текстовому полю (особенно о "Эффективности боевой машины").
    - В разделе "Продвинутые классы" теперь доступна опция "Третий класс", предоставляющая экран прокачки героя с выбором из трех навыков.
    - На экране героя добавлено новое описание и информация при щелчке по боевым машинам на кукле героя.
    - Изменена специализация Арлаха, теперь он специалист по "Эффективности боевой машины".
    - Теперь характеристики командира, зависящие от масштабирования, лучше отображаются в статистике командира в диалоговом окне командной строки.
    - Теперь общее значение силы некромантии можно увидеть в списке бонусов.
    - Добавлена эффективность боевых машин к 4 артефактам, что повышает их полезность.
    - Добавлен скрипт для расчета продолжительности игрового времени для каждого игрока.
    - Теперь применение заклинаний будет переопределять эффект предыдущих примененных заклинаний с текущей силой модификатора заклинаний. Теперь применяются корректные эффекты заклинаний.

    - Масштабирование всех артефактов командира уменьшено вдвое. Например: раньше вы получали +1% HP за каждый бой, теперь для этого нужно два боя.
    - Все артефакты командира можно снять, не теряя счетчик. Командир также может погибнуть, и бонусы не исчезнут. (Улучшение качества жизни ;-))
    - Увеличена сила Героев-Призывателей (бесплатное применение заклинаний на 25 уровне) и повышена эффективность тренера заклинаний.
    - Увеличено масштабирование урона Командира-Некроманта при воскрешении нежити.
    - Снижено масштабирование урона Командира-Бойца в зависимости от количества убийств, чтобы привести его в соответствие с другими классами.
    - Снижено масштабирование урона Огненного Мага.
    - Изменены детали накопления количества убийств Огненного Мага. Теперь количество убийств накапливается в зависимости от количества убитых юнитов и не получает случайного бонуса.
    - Класс Огненный Маг теперь получает выгоду от "Огненного Шара".
    - Командир-Конструктор может призывать только один раз за боевой раунд.
    - Несколько небольших комбинаций наборов артефактов были немного улучшены, теперь у них лучшие характеристики или бонусы. Теперь эти наборы дают лучшие бонусы для класса Воин, а не только для класса Маг.
    - Разведчики теперь имеют уменьшенное увеличение показателя уклонения при большом количестве убийств, достигающее максимума в 75%.
    - Немного снижено масштабирование способности "Пацифист" от командира "Астральный Дух".
    - Немного снижен бонусный урон баллисты от командира "Вождь Огров" до уровня 9+1: ранее он был уровня 6.
    - Снижен базовый урон баллисты с 4-6 до 3-5.
    - Удален двойной урон баллисты от навыка "Артиллерия". Отсутствие двойного урона теперь необходимо компенсировать с помощью "Эффективности мага".
    - Немного снижено масштабирование заклинания "Щит" для очень высоких значений "Силы магии" (на основе отзывов китайского сообщества).
    - Использование заклинания "Клонирование" требует дополнительных очков заклинаний, зависящих от количества и уровня цели. "Клонирование" может быть, безусловно, самым эффективным заклинанием по нанесению урона. Это немного уравновешивает затраты. Соотношение может измениться в будущем.
    - Немного снижена вероятность уклонения от атак с комплектом "Упряжь Пегаса".

    - Исправлена ошибка, из-за которой пассивная способность "Пожиратель душ" некорректно оживляла скелетов во время боя как для атакующей, так и для защищающейся стороны.
    - Исправлена ошибка, из-за которой одновременная экипировка нескольких свитков призыва полностью теряла свой эффект.
    - Исправлена ошибка, из-за которой скорость командира охотника в некоторых случаях обнулялась.
    - Исправлена ошибка, из-за которой способность командира отравлять, восстанавливать здоровье и блокировать могла исчезнуть после боя.
    - Исправлен расчет урона от перка "Блок" командира Тротсы (ограничение 100% шансом блокировать 95% урона).
    - Исправлена ошибка, из-за которой при смене профессии командира в бою количество убийств после повтора боя не возвращалось к значению до боя.
    - Исправлена ошибка с "Ботинками полярности" (они всегда давали бонус к передвижению), а не только в 33% случаев.
    - Исправлена ошибка, из-за которой призванные существа или клоны в некоторых случаях больше не теряют скорость во втором раунде боя.
    - Исправлен расчет емкости лечения для героя с ID 0 при игре на стороне защитников.
    - Исправлена ошибка, из-за которой усиление характеристик генерала предоставлялось каждый раз и игнорировало статус мастерства.
    - Исправлена ошибка, из-за которой Огненный Маг иногда не мог получить повышение до Древнего (спасибо 神月初雪).
    - Исправлена возможная ошибка при выборе класса Древнего командира на экране выбора (спасибо 神月初雪).
    - Исправлена ошибка, из-за которой таблица Тренера Заклинаний на экране героя не работала из-за отсутствия ID героя.
    - Исправлено поведение расчета урона от заклинаний при открытии книги заклинаний с карты Adv.
    - Исправлена ошибка с мастерством командира Мстителя (всегда было активно).
    - Исправлена ошибка, изучающая заклинания с помощью Орлиного Глаза теперь требует правильного уровня навыка EE. Ранее эта проверка игнорировалась.
    - Переработаны все эффекты оглушающего удара (снижены характеристики целей, используемых для магии Мастерской Резни, способностей командира Шамана и т. д.) (спасибо Archer30).
    - Способность "Меткий мундир" у машины снабжения теперь срабатывает после атаки в ближнем бою (а не до неё).
    - Способность "Магическая артиллерия" (из навыка EE) стрелять по врагам при входе на поле боя теперь не убивает всю группу врагов (минимум 1 единица здоровья). Это может предотвратить проблему зависания игры после войны в некоторых случаях.
    - Мифриловая броня, Острый меч и Кольцо "Глаз дракона" были переработаны таким образом, что отображаемые на экране значения урона теперь включают их увеличенный/уменьшенный урон.
    - Добавлен боевой журнал для естественного сопротивления друида.
    - Пассивная способность "Медаль заявления" теперь работает со всеми банками существ (включая пользовательские), ранее она работала только с некоторыми банками существ.
    - Волшебные грибы и опции поместья отключены по умолчанию в моде.
    - Восстановлено отсутствующее описание составного набора, которое было утеряно из-за изменений в моде GEM.
    - Исправлено исчезновение встроенной способности блокировки Торосара во время боя.
    - Исправлена частичная неработоспособность бонуса набора "Честь чемпионов" (спасибо TheInvisible).
    - Исправлено срабатывание удачи в бою. Отрицательная удача может по-прежнему оставаться проблемой после последних обновлений лаунчера.

#### Game Enhancement Mod:
- исправлена ошибка, возникающая при старте карт, с изменёнными постройками в городах;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - добавлена возможность отключить запрет на добавление боевых машин командирами в банках существ через ключ:
    ```
    "gbfe.disable_fix.npc.creature_banks_war_machines": bool
    ```

#### Other:
- новый Менеджер Модов (@SyDr) обновлён до версии 0.98.73, а имя исполняемого файла изменилось на "Mod Manager.exe":
    - Исправлены кое-какие ошибки;
    - Уменьшен размер .exe;
    - Обновлены языковые файлы (английский теперь тоже включает ключи, а не просто небольшой список категорий);
    - Добавлены кое-какие мелкие фичи;
    - Добавлена опция предупреждать либо информировать пост-фактум, какие моды были (ну или будут) выключены при автосортировке (по умолчанию отключено);
    - Тоже, что и пунктом выше, но при выключенной сортировке будет только показывать непосредственно несовместимые моды;
    - Убрана автоматическая проверка обновлений в портативной версии;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - исправлено сохранение функционала чит-кода на постройку всех зданий после выхода из игры;
    - исправлена невозможность воскресить Золотых Драконов (и прочих юнитов) заклинанием "Жертва" из-за иммунитета к заклинанию "Воскрешение" (заклинание 4-го уровня по умолчанию);
- в список генерации объектов для случайных карт добавлен объект "Гильдия Наёмников";

#### Game Enhancement Mod:
- обновлен плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.7.4:
    - отображение доступных построек теперь поддерживает индексы зданий выше 43 (поддержка новых зданий из плагина "new_towns.era";
    - небольшие исправления прокрутки карты;

#### ERA Scripts:
- Максимум вторичных навыков: исправлено обнуление количества вторичных навыков у героя при работе с некоторыми плагинами и модами при перезагрузке карты;

#### ERA ERM Framework:
- Добавлена новая функция "!?FU(Battle_RedrawGridShadow);", исправляющая отрисовку дальности хода отрядов в битве, если была изменена их скорость;


### Версия 2.232

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.32:
    - исправлено отображение имён и описаний артефактов;

#### ERA Scripts:
- Артефакты рюкзака: исправлена работа артефактов, приносящих золото;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - исправлено переполнение Опыта Существ при соединении или перемещении отрядов;
    - исправлена некорректная формула подсчёта опыта при перемещении героя в гарнизон города;

#### Game Enhancement Mod:
- удалён скрипт "gem_crexpo_fix.erm"
## рекомендуется начать новую игру или нажать F12 после загрузки старой

#### Other:
- оптимизированы pac-файлы для некоторых модов;
...
#### ERA Scripts:
- Сухопутная Навигация: переписан скрипт, исправлен некорректный подсчёт очков передвижения;

#### TrainerX:
- исправлен перерасчёт очков передвижения героя после закрытия диалога;


### Версия 2.228

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.31:
    - в диалог Настроек Генерации добавлен объект "Святилище";
    - исправлена генерация Заснеженного Рынка (теперь использует счётчик объектов обычного Рынка);

#### WoG Scripts:
- небольшие исправления замены объектов;

#### ERA ERM Framework:
- Добавлена новая функция "!?FU(DL_SetPlayerColor)", позволяющая установить корректный цвет игрока для кастомных диалогов (DL):
    ```
    !?FU(DL_SetPlayerColor);
    ; Установите правильный цвет фоновой рамки диалога. Вызовите DL:N перед попыткой использования этой функции.

    !#VA(dlgId:x);     ID DL-диалога;
    !#VA(playerId:x);  Опционально. ID  игрока, цвет которого надо установить. По умолчанию устанавливается цвет взаимодействующего с диалогом игрока;
    ```

#### Other:
- обновлёна мануал по отключению DEP (Help/DEP_tutorial.html);


### Версия 2.227

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.30:
    - теперь в диалоге Настроек Генерации отображаются все доступные def'ы для соответствующих типов и подтипов;
    - исправлена логика создания списков для генерации объектов;
    - повышена производительность;

#### WoG Scripts:
- ВоГификация:
    - теперь помещаемые скриптами объекты будут соответствовать почве, на которую они помещаются;
    - исправлено помещение объекта "Случайный Монстр" вместо артефакта "Книга Заклинаний";
- Улучшения за мифрил: исправлено описание для текста улучшения Монолитов;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - теперь чит-код для выдачи ресурсов также добавляет 100 единиц мифрила;
    
#### Game Enhancement Mod:
- обновлен плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.7.3:
    - исправлен метод обновления информации о количестве мифрила;

#### WoG Scripts:
- Улучшенные Командиры: исправлено показ сообщения с выбором ресурсов при подборе Камней Силы;


### Версия 2.225

#### WoG:
- обновлён плагин "Interface_MainMenuAPI.era" до версии 1.8:
    - теперь новые кнопки интерфейса главного меню скрываются при открытии меню "Авторы" и "Рекорды";
- обновлён плагин "ERA_MultilingualSupport.era" до версии 2.4:
    - добавлена совместимость с плагинами, изменяющими количество героев;


### Версия 2.224

#### WoG:
- обновлён плагин "Interface_MainMenuAPI.era" до версии 1.7:
    - исправлено мерцание кнопок интерфейса главного меню


### Версия 2.223

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.29:
    - исправлена невозможность генерировать новые объекты;
- исправлен цвет игрока для отображения разрушенной Сельской Управы;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - теперь для объектов со свойством "плоский" отрисовывается тень на Карте Приключений;
    - теперь при разрушении зданий в городах без фортов на Карте Приключений будет отображаться Сельская Управа;
    
#### Game Enhancement Mod:
- обновлен плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.7.2:
    - добавлено отображение подсказки о строительстве в диалог просмотра города при нажатии на ПКМ и в окно самого города;


### Версия 2.221

#### Game Enhancement Mod:
- удалено отображение символа "1" на верхней иконке в списке городов;


### Версия 2.220

#### Game Enhancement Mod:
- обновлен плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.7.1:
    - исправлено исчезновение подсказки о построенных зданиях в городах при полном завершении движения героев;
    - отключен показ подсказки во время хода ИИ во избежании багов отрисовки;
...
#### WoG:
- обновлён плагин "wog native dialogs.era":
    - Теперь в диалогах Чёрного рынка и Торговца Артефактами доступно открытие окна активного героя;

#### WoG Scripts:
- Неделя монстров: теперь при наступлении новой недели отображается соответствующая контенту картинка;
...
#### ERA
    - ВНИМАНИЕ! примеры использования всех нововведений находятся в файле "/Help/Era manual/era iii changelog.txt" или просто кликните по версии ERA в главном меню игры;
- ## обновление ядра era.dll до версии 3.9.27:
    - Исправлена ошибка рассинхронизации в сетевых боях PvP, возникавшая, когда процессор одного клиента поддерживал SSE 4.2, а другого — нет. Две системы использовали разные алгоритмы хеширования CRC‑32, что приводило к несовпадению обновлений состояния. Патч нормализует процедуру хеширования и обновляет экспортированную функцию `Hash32`, чтобы она выдавала одинаковые результаты на всех платформах независимо от мощности процессора.
    - Исправлено обрезание текста в многострочных текстовых диалоговых окнах.

#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - исправлен вылет на ход ИИ при расчёте морали в армии с Альянсом Ангелов;

#### Game Enhancement Mod:
- обновлен плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.7.0:
    - В окна Карты Приключений и Просмотра Города добавлено дополнительные иконки статуса строительства зданий:
        - Всё построено — жёлтая галка;
        - Доступны постройки для строительства, но не хватает денег — перечёркнутые золотые монеты;
        - Доступны постройки и хватает ресурсов — значок скрыт;
        - управляется через json-ключ "gem_plugin.building_hints.enable";

### Версия 2.217

#### WoG:
- обновлён плагин "wog native dialogs.era":
    - исправлены утечки памяти и вылеты из диалогов с динамическим изменением шрифтов;
    - для закрытия меню WoG-опций добавлена горячая клавиша "Esc";
- обновлён плагин "Interface_MainMenuAPI.era" до версии 1.6:
    - добавлена возможность лёгкой сортировки добавляемых кнопок:
    ```
    Для виджетов добавлены 2 флага:
    ON_TOP - для отображения виджета ближе к началу списка;
    AT_BOTTOM - для отображения виджетов ближе к концу списка;
    ```
    - обновлена API по относительному каталогу "Tools/Era/SDK/MainMenuAPI.hpp";
    - мелкие исправления;
- обновлён плагин "Assembly_MainPlugin.era" до версии 1.82:
    - добавлена поддержка нового API;
    - теперь кнопка "Уведомления" в главном меню отображается как можно ниже;
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.29:
    - добавлена поддержка нового API;
    - теперь кнопка "Настройки Генерации" в главном меню отображается как можно ниже и только в меню "Одиночная Игра";
 - обновлён плагин "ERA_MultilingualSupport.era" до версии 2.3:
    - добавлена поддержка нового API;
    - теперь кнопка "Язык" в главном меню отображается как можно выше;
...
#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - добавлено ещё больше совместимости с плагином "Typhon";
    - исправлено получение существами опыта не из битвы (опция "старт героев с 5 уровнем", например);
    - исправлена невозможность существам получать опыт, если уровень героя достиг максимального для данной карты;

#### Game Enhancement Mod:
- обновлен плагин "Gameplay_GameplayEnhancementsPlugin.era" до версии 1.6.4:
    - теперь в режиме "Hot-Seat" добавленные элементы интерфейса корректно окрашиваются в цвет игрока-человека (ещё до принятия хода);
- возвращено отображение сообщения о достижении героем максимального уровня. Показываться будешь лишь раз для каждого героя;


### Версия 2.215

#### ERA ERM Framework:
- Добавлена новая функция, позволяющая получить список активных модов (названия папок модами):
    ```
    !?FU(GetModList);
    !#VA(arrayId:x); возвращает индекс массива строк с названиями (как в list.txt)
    !#VA(toLower:x); Опционально: сохранить названия модов в нижнем регистре; по умолчанию "НЕТ"
    !#VA(reverse:x); Опционально: развернуть список модов в массиве; по умолчанию "НЕТ"
    ```

#### Game Enhancement Mod:
- Восстановлено оптимальное получение версии игры;
- Мелкие исправления внутренних функций;

#### WoG:
- обновлён плагин "game bug fixes extended.dll":
    - исправлен вылет при атаки существ в ближнем бою с активным модом "ERA+";
...
#### WoG Graphics Fix Lite:
- мод обновлён до версии 2.23.0;


### Версия 2.213

#### WoG Scripts:
- Волшебство I: исправлено повреждение данных некоторых объектов при удалённом посещении;


### Версия 2.212

#### ERA ERM Framework:
- исправлены константы для индексов классов героев;

#### WoG Graphics Fix Lite:
- исправлена маска проходимости для объекта "Городские Врата";


### Версия 2.211

#### Other:
- новый Менеджер Модов (@SyDr) обновлён до версии 0.98.71:
    - добавлена совместимость с Heroes Launcher v1.2.0;
    - добавлена опция "Создать новый мод" во вкладке "Инструменты";


### Версия 2.210

#### WoG:
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.28:
    - добавлена работоспособность плагина без HD Mod;
    - исправлено дублирование банков существ в диалоге настроек объектов;
    - теперь артефакты, являющиеся частью сборников, не могут быть объектом заданий на случайных картах;
- небольшие исправления локализации;

#### Game Enhancement Mod:
- удалён дублирующийся код для старых исправлений и изменений;
...
- обновлён плагин "RMG_CustomizeObjectProperties.era" до версии 1.26:
    - переписана логика для добавления Экстендеров для будущей поддержки модульности плагина;
...
#### WoG Scripts:
- Улучшенные Военные Машины III: в окно героя и встречи добавлены цифровые отображения текущего количества боевых машин;
...
- Исправлено несколько проблем с классом командира Fire Mage:
    ...
    - Они не могут перейти в класс Ancient, если он не разблокирован в игре.
...
#### ERA ERM Framework:
- добавлен адрес массива структур Командиров;
...
- Улучшенные Военные Машины III: Исправлена возможность лечения враждебных стеков;
...
- Исправлена подсказка поддержки горячего кресла для Улучшенных Военных Машин III
...
#### TrainerX:
- Добавлен режим Бога. Нажмите Alt-клик по портрету героя, чтобы максимально использовать Армию/Движение/Ману/Полет/Основные навыки/Вторичные навыки/Заклинания/Видение карты/Ресурсы - спасибо Archer30
